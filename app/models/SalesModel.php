...
class SalesModel
{
    ...
    public function create(array $d): int
    {
        ...
        $nextSalesKey = (int)Database::fetchValue("SELECT COALESCE(MAX(sales_key),0)+1 FROM dw__fact_sales");
        $nextSaleId   = (int)Database::fetchValue("SELECT COALESCE(MAX(sale_id),0)+1 FROM dw__fact_sales");

        $stmt = Database::connection()->prepare(
            "INSERT INTO dw__fact_sales
                (sales_key, sale_id, date_key, product_key, store_key,
                 units, unit_price, unit_cost, revenue, cogs, gross_profit, margin_pct)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"
        );

        $stmt->execute([
            $nextSalesKey, $nextSaleId, $date['date_key'], $prod['product_key'], $store['store_key'],
    ...
    {
        $row = Database::fetchOne("SELECT * FROM dw__fact_sales WHERE sale_id=?", [$salesKey]);
        if (!$row) return false;
        ...
        if (!$fullDate) {
            $dateRow  = Database::fetchOne("SELECT full_date FROM dw__dim_date WHERE date_key=?", [$row['date_key']]);
            $fullDate = $dateRow['full_date'] ?? null;
        }

        $date = Database::fetchOne("SELECT date_key FROM dw__dim_date WHERE full_date=?", [$fullDate]);

        $stmt = Database::connection()->prepare(
            "UPDATE dw__fact_sales SET
                date_key=?, product_key=?, store_key=?, units=?, unit_price=?, unit_cost=?,
