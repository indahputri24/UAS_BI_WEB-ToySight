...
class ProductModel
{
    public function paginate(string $search = '', string $category = '', int $page = 1, int $perPage = 12): array
    {
        ...
        $rows = Database::fetchAll(
            "SELECT p.*,
                    (SELECT COALESCE(SUM(stock_on_hand),0) FROM dw__fact_inventory i WHERE i.product_key=p.product_key) AS total_stock,
                    (SELECT COALESCE(SUM(units),0) FROM dw__fact_sales s WHERE s.product_key=p.product_key)            AS total_units_sold,
                    (SELECT COALESCE(ROUND(SUM(revenue),2),0) FROM dw__fact_sales s WHERE s.product_key=p.product_key) AS total_revenue
